...
# 弹出输入框，询问延长时间
user_input = simpledialog.askstring("输入延长时间", "延长多少毫秒？", initialvalue="00")

# 将用户输入转换为整数（单位为毫秒），并进行错误处理
try:
    time_offset = int(user_input)  # 默认是“00”
except ValueError:
    print("输入无效，默认为0毫秒。")
    time_offset = 0
...
# 读取output2.lrc文件
try:
    with open('output2.lrc', 'r', encoding='utf-8') as file:
        lines = file.readlines()
except FileNotFoundError:
    print("错误：未找到文件 'output2.lrc'。请确保文件存在于当前目录。")
    exit(1)
...
# 写入output3.lrc文件
try:
    with open('output3.lrc', 'w', encoding='utf-8') as file:
        for line in lines:
            adjusted_line = adjust_time(line, time_offset)
            file.write(adjusted_line)
    print("时间轴调整完成，输出文件为 'output3.lrc'。")
except Exception as e:
    print(f"写入文件时出错: {e}")
    exit(1)
