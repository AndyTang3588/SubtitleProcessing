...
def main():
    ...
    if os.path.exists(original_lrc):
        backup_file(original_lrc)
    else:
        print(f"输入文件 {original_lrc} 未找到，无法创建备份。")
        exit(1)

    # 从 output3.lrc 文件中读取字幕
    input_data = read_input_file(original_lrc)
    ...
    for i, (time_str, subtitle) in enumerate(matches, 1):
        ...
        if i < len(matches):
            ...
            if len(next_parts) == 3:  # hh:mm:ss
                hours = int(next_parts[0])
                minutes = int(next_parts[1])
                seconds = float(next_parts[2])
            elif len(next_parts) == 2:  # mm:ss
                hours = 0
                minutes = int(next_parts[0])
                seconds = float(next_parts[1])
            else:
        ...
        # 转换时间
        start_time_srt = convert_to_srt_time(f"[{int(start_seconds // 3600):02}:{int((start_seconds % 3600) // 60):02}:{start_seconds % 60:.2f}]")
        end_time_srt = convert_to_srt_time(f"[{int(end_seconds // 3600):02}:{int((end_seconds % 3600) // 60):02}:{end_seconds % 60:.2f}]")
